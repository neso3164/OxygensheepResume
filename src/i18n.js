...
const dict = {
  'zh-Hant': {
    ...
    work_d_desc: '報價單產品數量太龐大翻閱太耗時(近2000項>透過自動化處理後更新報價單並且提供關鍵字查詢報價>每次報價單更新或需要查價時節省下很多時間<P/>客戶預約時如果不在電腦前就要拿紙本記或是輸入手機備忘錄>能夠藉由自動化流程運算加入行事曆(包含手機)>省去的繁雜的紙本以及要反覆確認有沒有漏掉輸入行程<P/>以往分析營業情況都需要整理彙整資料進行分析>藉由自動化處理能夠輸入時直接以正確的格式存入檔案.分析時可以直接使用>避免了繁瑣的資料清洗<P/>',
